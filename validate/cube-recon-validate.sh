#!/bin/bash
#
# Run tests on CubeRecon.  The tests kept in the fast-tests
...
#
# This needs to be run in the validate subdirectory.  It assumes that
# CubeRecon has been setup and all the necessary packages are in the
# path.
...
mkdir ${OUTPUT_DIR}
for i in $(find ${TESTS} -name "[0-9]*" -type f -executable); do
    if [ -x ${i} ]; then
        LOG=$(basename $i)
